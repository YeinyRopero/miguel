...
<tr><th>Asignar a vendedor</th>
<td><select class="form-control" name="id_usuario">

  <?php 

  include('conexion.php');

$sql="SELECT * FROM maps.usuario where id_perfil='2' ";
$query=pg_query($sql);
$total = pg_num_rows($query);
  ...
  if($total>0){
    ...
    $arreglo = pg_fetch_array($query);
    ?>
    <option value="<?php echo "$arreglo[id_usuario]"; ?>"> <?php echo "$arreglo[name_usuario]"; ?> </option>
<?php
} //cierro for
} //cieroo if
?>
...
<tr><td colspan="2"><input type="button" class="btn btn-primary" onclick="guardar()"  value="Guardar Ubicación"/></td></tr>
    ...
    <div id="mapa">   <!-- contenedor del mapa --> 
      <br><br><br><br>
